        ...
        <table className="text-left w-full border-collapse table-auto">
            ...
            <tbody>
                <tr className="border-2 border-white even:bg-gray-100 odd:bg-gray-300 h-full align-top">
                    ...
                    <td className="px-6 py-3 flex items-center justify-start h-full">
                        ✅ No issues to report
                    </td>
                </tr>
            </tbody>
        ...
        <table className="text-left w-full border-collapse table-auto">
            ...
            <tbody>
                {sortedValidations.map((validation, index) => (
                    <tr
                        key={index}
                        className="border-2 border-white even:bg-gray-100 odd:bg-gray-300 h-full align-top"
                    >
                        <td className="px-6 py-2 w-36 border-r-2 border-white align-middle">
                            <div className="flex items-center justify-center h-full">
                        ...
                        <td className="px-6 py-3 flex flex-col justify-start h-full">
                            ...
                            {validation.refs && (
                                <div className="text-xs align-middle inline-flex gap-3">
                                    {validation.refs?.hl7 && (
                                        <RefLink href={validation.refs?.hl7} type="hl7" />
                                    )}
                                    {validation.refs?.simplifier && (
                                        <RefLink href={validation.refs?.simplifier} type="simplifier" />
                                    )}
                                    {validation.refs?.nav && (
                                        <RefLink href={validation.refs?.nav} type="nav" />
                                    )}
                                </div>
                            )}
                        </td>
