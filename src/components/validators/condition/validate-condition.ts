...
export function validateCondition(conditions: Condition[]): Validation[] {
    ...
    conditions.forEach((condition, index) => {
        ...
        if (!condition.code) {
            ...
            const validCodings = condition.code.coding.filter(
                (code) => code.system === ICD10 || code.system === ICPC2,
            )

            if (validCodings.length === 0) {
                validator.error(
                    `[${index}] The Condition code object does not contain a valid coding reference`,
                    {
                        hl7: hl7Refs.condition,
                        nav: navRefs.condition,
                    },
