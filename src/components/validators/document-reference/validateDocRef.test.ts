...
describe('validateDocRef', () => {
    it('should validate our own example structure', () => {
        const validations = validateDocumentReference(workingDocumentReference, 'foo-bar-baz').filter((it) =>
            isBad(it),
        )
    ...
    it('should complain if ID returned is different than ID provided in payload', () => {
        const validations = validateDocumentReference(workingDocumentReference, 'wrong-id').filter((it) =>
            isBad(it),
        )
