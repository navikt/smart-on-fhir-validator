...
1. [**Tell this app how to authenticate with your EHR**](#step-1-add-your-ehr-to-the-config-file)
2. [**Let Nav's cluster reach your EHR's servers**](#step-2-allow-network-access)
3. [**Configure your EHR to launch this app**](#step-3-launch-from-your-ehr)
4. [**Read the result**](#step-4-read-the-report)

Steps 1 and 2 happen in the same pull request, against one file. No SMART or OAuth background is
assumed below. If a term is unfamiliar, it's explained the first time it's used.

> **In a hurry, and your authorization server supports Dynamic Client Registration (RFC 7591)?**
> You can skip Step 1. This app registers itself automatically as a public client on first launch.
> You still need Step 2 (network access). That includes the host of your `registration_endpoint`,
> which may differ from your other hosts. Nothing works without it. Then go straight to
> [Step 3](#step-3-launch-from-your-ehr).

### Step 1: Add your EHR to the config file

This app is an **OAuth client**. That means it's a piece of software (this one) that logs in via
your EHR's **authorization server**. That's the part of your EHR system that issues access tokens;
it's separate from the FHIR server that holds patient data. Before it will exchange tokens with
this app, it needs two things from you:

- This app's identity (a `clientId`)
- How this app proves that identity: no secret, a shared secret, or a signed key. The three
  options below cover each one.

You give this information by editing one file: **[`.nais/nais-dev.yaml`](.nais/nais-dev.yaml)**.

Open that file and find the block that starts with `SMART_ISSUERS`. Its value is a JSON array.
It's empty if no vendor has registered yet. Otherwise it contains one object per already-registered
vendor. Before any vendor has registered, it looks like this:
...
You'll add a JSON object describing your EHR to that array. If the array is currently empty,
that turns `[]` into a one-element array. If it already has entries, you append yours to the
end. There are three possible shapes for that object. Which one you need depends on how your
authorization server expects a client to identify itself. **Pick the one that matches your setup.**
If you're not sure which one applies, ask whoever configured your EHR's SMART/OAuth settings.
You can also open an ["Onboard an EHR" issue](../../issues/new?template=onboard-ehr.yml) and a
maintainer will help.

Every field below is a name or a public identifier, never a secret. That makes this whole file
safe to edit directly in the GitHub web UI, and the diff safe to make public.

**Option A: no client secret at all ("public" client, `authType: "public"`).** Use this if your
authorization server doesn't ask for a client secret. Replay protection instead comes from PKCE
("Proof Key for Code Exchange"). PKCE is a one-time secret this app generates and verifies for
each login, so no long-lived secret is needed. This app always uses it automatically. There's
nothing extra to configure here, or on your EHR's side, beyond selecting "public"/PKCE as the
...
This example omits `method`. It defaults to `client_secret_basic`, meaning the secret is sent in
an `Authorization` header. If your authorization server administrator instead told you to send
the secret in the POST body, add `"method": "client_secret_post"` to the object above. Ask them
if you're not sure which one applies.

`clientSecretEnv` is **a name you invent for the secret, not the secret itself.** It must start
with `SMART_CLIENT_SECRET_`, for example `SMART_CLIENT_SECRET_ACME`. Other vendors are registered
in the same file, so it **must be unique**. Pick something that includes your vendor name. Don't
use a generic name like `SMART_CLIENT_SECRET_EHR`; another entry might already use it, and CI
rejects duplicates. Never paste the actual secret value into this file, a commit, or your pull
request description. Once merged, it is public and permanent. After your PR is open, ask a
maintainer (comment `@navikt/helseopplysninger` on the PR) to set the real secret value in Nav's
deployment secret. See [Step 1b](#step-1b-if-you-chose-a-shared-secret) below.
...
The exact fields allowed for each `authType`, and what happens if you get one wrong, are defined
in [`src/core/config/issuers.ts`](src/core/config/issuers.ts). Your pull request is checked
automatically against that same schema: `src/core/config/manifest-issuers.test.ts` in CI, or run
`yarn test` yourself. A typo or a missing field fails your PR with a clear error, instead of
...
somewhere your entry can't reach: the `smart-on-fhir-validator-clients` Kubernetes secret, not
this file. Once your PR from Step 1 is open, comment on it (or ping
`@navikt/helseopplysninger`) with the name you chose. Then, through a secure or private channel
of your choosing, send the secret value itself. A maintainer sets it before merging. If your
authorization server supports Option A or C instead, you can skip this step entirely. That's why
...
launch will fail at runtime, with a network/egress error that's confusing to debug from the report
alone. Do both steps in the same pull request.

In the same file, **[`.nais/nais-dev.yaml`](.nais/nais-dev.yaml)**, find
`spec.accessPolicy.outbound.external` (near the top of the file). Add one `- host: "..."` line
for every hostname this app needs to call to complete your SMART launch:

- Your FHIR base URL's host
- Your authorization endpoint's host
- Your token endpoint's host
- Your JWKS URI's host, if it differs from the others

These are frequently *not* all the same hostname. For example, FHIR might be on
`fhir.acme.example.com` while authorization and tokens are on `auth.acme.example.com`. Don't
assume one host covers everything. If you're not sure which hosts are involved, ask your EHR's
SMART/OAuth administrator. You can also fetch
...
```yaml
external:
  # other vendors' hosts, if any, are already listed above this comment; add yours below them:
...
If you did both steps correctly for a fictional vendor "Acme EHR" using a shared secret
(Option B), your diff against `.nais/nais-dev.yaml` looks like this, with two additions in one PR.
This is shown assuming yours is the first entry. If the file already has other vendors' hosts or
entries, yours is appended alongside them instead:
...
- [ ] Every hostname your integration touches (FHIR, authorization, token, JWKS) is listed under
      `spec.accessPolicy.outbound.external`. This is true even if some of them repeat a hostname
      already in the list from another vendor.
...
configure your EHR to open this app the same way it would open any other SMART app for a
clinician. From your EHR, that means directing the browser to this app's launch URL, **with two
query parameters your EHR fills in itself**:

```text
{this app's origin}/launch?iss=<your exact fhirBaseUrl>&launch=<opaque value your EHR generates>
```

- `iss` is short for "issuer" in the SMART spec; it has nothing to do with `authType`. It must be
  **exactly** the `fhirBaseUrl` string you wrote in Step 1. That's the FHIR server base URL, not
  the authorization or token endpoint.
- `launch` is an opaque identifier your EHR generates per session. It tells this app which
...
Beyond the URL, your EHR's own SMART/OAuth client configuration needs to match what you
registered in Step 1 exactly:
...
Which "client authentication method" to pick on your EHR's side depends on the `authType` you set
in Step 1:

- `public` → "none" / public client, plus PKCE
...
of scopes than requested is not penalised for it. The Scopes and Launch Context sections report
what was granted, not what was asked for. If launch fails immediately, before you even see a
report, with an error like `missing_launch` or `invalid_iss`, check the query parameters above
first. If it fails after you log in at your EHR, the `clientId`/auth method almost certainly
doesn't match what's in `.nais/nais-dev.yaml`. See
[docs/troubleshooting.md](docs/troubleshooting.md).

### Step 4: Read the report

After a successful launch, this app redirects to `/report`. It lists every check it ran against
